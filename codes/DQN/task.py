...
def MT_train(env: gym.Env, agent:DQN, cfg):
    ...
    for episode_i in range(cfg["episodes"]):
        ...
        while True:
            ...
            m_steps_buffer.add(s,s_,a,r,done)
...
def eval(env_name: str, agent:DQN, cfg):
    env:gym.Env = gym.make(env_name, render_mode="human")
    name = cfg["env_name"]+"_"+str(cfg["layer_num"])+"_best.pt"
...
def main():
    ...
    agent = DQN(
        ...
        double=agent_dict["double"],
        m_steps=cfg["m_steps"]
    )
    

    print(agent.q_net)
    MT_train(env,agent,cfg)
    #env.close()
